...
public interface AuthRequestCallback {
    Event<AuthRequestCallback> LOGIN = EventFactory.createArrayBacked(AuthRequestCallback.class,
    ...
    Event<AuthRequestCallback> SIGNUP = EventFactory.createArrayBacked(AuthRequestCallback.class,
            listeners -> (player, password) -> {
        for (AuthRequestCallback listener : listeners) {
            listener.offer(player, password);
        }
